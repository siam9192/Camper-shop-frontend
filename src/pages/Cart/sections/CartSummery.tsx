...
const CartSummery = ({ products }: TCartSummeryProps) => {
  const subtotal = products.reduce(
    (prev: number, current: TCartProduct) =>
  ...
  const stockOutProduct = products.find(
  ...
  return (
    <div className="bg-gray-100 rounded-lg p-5">
      ...
      <button
        ...
        disabled={stockOutProduct ? true : false}
        className="bg-button_color text-white disabled:bg-gray-600 py-2 px-4 rounded-lg mt-4 w-full"
      >
        Checkout
      </button>
    </div>
