...
const Select = ({onChange,options}:TSelectProps) => {
  return (
    ...
    <select onChange={onChange} data-hs-select='{
    ...
    {
        options.map((option,index)=><option value={option.value} defaultChecked={option.isChecked} key={index}>{option.display}</option>)
    }
    </select>
